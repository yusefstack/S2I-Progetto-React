...
const ResercheadRecipes = () => {
    const researchedRecipes = useSelector(state => state.recipes.researchedRecipes)

    const researchedRecipesToDisplay = researchedRecipes.map((recipe) => {
    return (
        <div 
            key={uuidv4()} 
        ...
        >
            <figure className="bg-gray-100 px-10 pt-10">
                <img src={recipe.image} alt="recipe" className="bg-gray-100 rounded-xl" />
            </figure>
            <div className="bg-gray-100 card-body rounded-b-2xl items-center text-center">
                <h2 className="bg-gray-100 card-title h-10 mb-3 max-md:text-sm">{recipe.title}</h2>
                <div className="bg-gray-100 card-actions">
                    <Link 
            ...
            <>
                <div className="w-full h-screen flex flex-col lg:flex-row items-center justify-center space-y-16 lg:space-y-0 space-x-8 2xl:space-x-0">
                    <div className="w-full lg:w-1/2 flex flex-col items-center justify-center lg:px-2 xl:px-0 text-center">
                        <p className="text-lg md:text-xl lg:text-2xl text-gray-500 my-12">Sorry, the recipe you are looking for could not be found.</p>
                        <Link 
                            to="/" 
                            className='btn'
                        >
...
return (
    <>
        ...
        <div>
            <h1 className='text-6xl ml-10 mb-10 text-bold'>Results</h1>
            <div className="w-5/6 m-auto grid grid-cols-3 gap-20 max-xl:block">
                {researchedRecipesToDisplay}
            </div>
        </div>
...
export default ResercheadRecipes